...
function home(req){const key=encodeURIComponent(VIEWER_KEY); return `<!doctype html><html lang="ru"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1,viewport-fit=cover"><title>AllSaveMod CRM</title><style>:root{--bg:#0b141a;--panel:#111b21;--panel2:#17232b;--card:#182830;--text:#e9edef;--muted:#8696a0;--line:rgba(255,255,255,.08);--accent:#2aabee;--green:#00a884}*{box-sizing:border-box}body{margin:0;background:var(--bg);color:var(--text);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,Arial,sans-serif}.app{min-height:100vh;display:grid;grid-template-columns:minmax(320px,430px) 1fr}.side{background:var(--panel);border-right:1px solid var(--line);min-height:100vh}.top{position:sticky;top:0;z-index:5;background:rgba(17,27,33,.94);backdrop-filter:blur(16px);padding:max(14px,env(safe-area-inset-top)) 14px 12px;border-bottom:1px solid var(--line)}.h{display:flex;align-items:center;justify-content:space-between;gap:12px}.brand{display:flex;align-items:center;gap:10px}.logo{width:42px;height:42px;border-radius:50%;display:grid;place-items:center;background:linear-gradient(135deg,var(--accent),#5dd2ff);font-weight:900;color:#001722}.title{font-size:20px;font-weight:900}.count{font-size:13px;color:var(--muted);margin-top:2px}.search{width:100%;height:44px;margin-top:12px;border:0;outline:0;border-radius:12px;background:#202c33;color:var(--text);padding:0 14px;font-size:15px}.toolbar{display:flex;gap:8px;margin-top:10px;flex-wrap:wrap}.btn{color:#dff4ff;background:#202c33;border:1px solid var(--line);border-radius:12px;padding:9px 12px;font-weight:800;text-decoration:none;font-size:13px}.list{padding:8px}.card{display:grid;grid-template-columns:52px 1fr auto;gap:12px;padding:11px;border-radius:14px;color:inherit;text-decoration:none;align-items:center;border-bottom:1px solid rgba(255,255,255,.04)}.card:hover{background:#1f2c33}.card.unread{background:rgba(0,168,132,.12)}.avatar{width:52px;height:52px;border-radius:50%;display:grid;place-items:center;background:#2aabee;color:white;font-weight:900}.name{font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.last{margin-top:4px;color:var(--muted);font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.meta{text-align:right;color:var(--muted);font-size:12px}.badge{margin-top:5px;display:inline-block;background:var(--green);color:#04251e;border-radius:999px;padding:2px 7px;font-size:11px;font-weight:900}.empty{color:var(--muted);text-align:center;padding:35px 14px}.hero{display:flex;align-items:center;justify-content:center;min-height:100vh;background:radial-gradient(circle at 50% 20%,rgba(42,171,238,.14),transparent 36%),var(--bg)}.heroBox{text-align:center;color:var(--muted);max-width:430px;padding:20px}.heroIcon{font-size:58px;margin-bottom:12px}@media(max-width:760px){.app{display:block}.hero{display:none}.side{min-height:100vh}}</style></head><body><div class="app"><aside class="side"><div class="top"><div class="h"><div class="brand"><div class="logo">A</div><div><div class="title">AllSave CRM</div><div class="count" id="count">Загрузка...</div></div></div><a class="btn" href="/export?key=${key}">Backup</a></div><input id="search" class="search" placeholder="Поиск по чатам..."><div class="toolbar"><a class="btn" href="/import?key=${key}">Import</a><a class="btn" href="javascript:load(true)">Обновить</a></div></div><div class="list" id="list"><div class="empty">Загрузка...</div></div></aside><main class="hero"><div class="heroBox"><div class="heroIcon">💬</div><h2>Выбери чат</h2><p>Сообщения откроются как в Telegram Web. Обновление без мигания и сброса скролла.</p></div></main></div><script>const KEY=${JSON.stringify(VIEWER_KEY)},API='/api/chat?key='+encodeURIComponent(KEY);let dialogs=[],lastHash='';function esc(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[m]))}function initials(s){s=String(s||'?').trim();return s.split(/\s+/).slice(0,2).map(x=>x[0]||'').join('').toUpperCase()||'?'}function unread(d){let k='read_at:'+(d.short_id||d.id),r=Number(localStorage.getItem(k)||0);return Number(d.updated_at||0)>r}function render(){const q=document.getElementById('search').value.trim().toLowerCase();const list=document.getElementById('list');const items=dialogs.filter(d=>!q||[d.title,d.shortTitle,d.id,d.short_id,d.lastText].filter(Boolean).some(x=>String(x).toLowerCase().includes(q)));document.getElementById('count').textContent=dialogs.length+' диалогов';if(!items.length){list.innerHTML='<div class="empty">Ничего не найдено</div>';return}list.innerHTML=items.map(d=>{const href=d.short_id?'/c?s='+encodeURIComponent(d.short_id)+'&key='+encodeURIComponent(KEY):'/chat?id='+encodeURIComponent(d.id)+'&key='+encodeURIComponent(KEY);const u=unread(d);return '<a class="card '+(u?'unread':'')+'" href="'+href+'"><div class="avatar">'+esc(initials(d.shortTitle||d.title))+'</div><div style="min-width:0"><div class="name">'+esc(d.shortTitle||d.title||d.id)+'</div><div class="last">'+esc(d.lastText||('Сообщений: '+(d.count||0)))+'</div>'+(u?'<div class="badge">NEW</div>':'')+'</div><div class="meta">'+esc(d.count||0)+'</div></a>'}).join('')}async function load(force=false){try{const r=await fetch(API,{cache:'no-store'});const d=await r.json();if(!d.ok){document.getElementById('list').innerHTML='<div class="empty">Ошибка загрузки</div>';return}const h=JSON.stringify(d.dialogs||[]);if(!force&&h===lastHash)return;lastHash=h;dialogs=d.dialogs||[];render()}catch(e){document.getElementById('list').innerHTML='<div class="empty">Ошибка сети</div>'}}document.getElementById('search').addEventListener('input',render);load(true);setInterval(()=>{if(!document.hidden)load(false)},4000)</script></body></html>`}
...
app.get('/chat',(req,res)=> okKey(req)?html(res,chatPage(req)):html(res,blocked(),403));
